...
if __name__ == "__main__":
    ...
    print("\n[STANDARD] Code Rate R=0.5 (64 data + 64 parity)")
    
    data_r05 = create_ldpc_dataset(
        num_samples=5000,
        n=128,
        code_rate=0.5,
        save_path='ldpc_codewords_r05.pt',
        seed=42
    )
    
    print("\nDataset Verification:")
    print(f"  Shape: {data_r05.shape}")
    print(f"  Values: {torch.unique(data_r05).tolist()}")
    print(f"\nFirst 3 codewords:")
    for i in range(3):
        print(f"  Codeword {i+1}: {data_r05[i].numpy()}")
